...
> 用户的创建与用户认证，在之前的[Gerrit_HTTP](gerrit-source.md)和[Gerrit_LDAP](gerrit-ldap.md)已讲解过，在此简单介绍用户和群组的权限规划
...
● gerrit.service - gerrit Server
   Loaded: loaded (/etc/systemd/system/gerrit.service; enabled; vendor preset: disabled)
   Active: active (running) since Thu 2022-07-14 17:12:24 CST; 3s ago
  Process: 2583 ExecStop=/usr/local/gerrit/review_site/bin/gerrit.sh stop (code=exited, status=0/SUCCESS)
  Process: 2714 ExecStart=/usr/local/gerrit/review_site/bin/gerrit.sh start (code=exited, status=0/SUCCESS)
 Main PID: 2765 (java)
   CGroup: /system.slice/gerrit.service
           └─2765 GerritCodeReview -Dflogger.backend_factory=com.google.common.flogger.backend.log4j.Log4jBackendFactory#getInstance -Dflogger.logging_context=com.google.gerrit.server...

Jul 14 17:12:14 m2-debug systemd[1]: Starting gerrit Server...
Jul 14 17:12:14 m2-debug gerrit.sh[2714]: Starting Gerrit Code Review: WARNING: Could not adjust Gerrit's process for the kernel's out-of-memory killer.
Jul 14 17:12:14 m2-debug gerrit.sh[2714]: This may be caused by /usr/local/gerrit/review_site/bin/gerrit.sh not being run as root.
Jul 14 17:12:14 m2-debug gerrit.sh[2714]: Consider changing the OOM score adjustment manually for Gerrit's PID=2765 with e.g.:
Jul 14 17:12:14 m2-debug gerrit.sh[2714]: echo '-1000' | sudo tee /proc/2765/oom_score_adj
Jul 14 17:12:24 m2-debug systemd[1]: Started gerrit Server.
```
